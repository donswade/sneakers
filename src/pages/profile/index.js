...
const ProfilePage = ({ setBoughtCards, items, input, showCartSearched, searchedCards, renderCards }) => {
  // const [sameCart, setSameCart] = React.useState([
  //   {
  //     "orderNumber": "0001",
    ...
    return data.map((element, index) => {
      return <div className={style.order} key={index}>
        <div className={style.top}>
          ...
          <div onClick={() => setOpen(index)} className={element.isOpen ? `${style.top__button} ${style.top__buttonOpen}` : `${style.top__button} ${style.top__buttonClose}`}><img src={arrow} alt="arrow back" /></div>
  ...
  return (
    <>
      ...
      <div className={style.main}>
        ...
        {items.length > 0 ? <ShowOrders data={items} /> : <NothingFounded title="Пусто :(" description={`Ти ще нічого не вподобав`} />}
